...
dag = DAG(
    USERNAME + '_final_dwh_etl_payment',
...
final_fill_ods_hashed = PostgresOperator(
    ...
    sql="""
        ...
        billing_period_pk, payment_pk, pay_pk, user_account_pk, user_hashdif, payment_hashdif, effective_from, '{{ execution_date }}'::TIMESTAMP FROM araev.final_ods_payment_hashed 
        WHERE EXTRACT(YEAR FROM pay_date::DATE) = {{ execution_date.year }}
    """
)
